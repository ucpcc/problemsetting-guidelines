...
1. 모든 문제의 디스크립션은 맞춤법을 지키도록 작성하는 것을 권장합니다. 단, 극히 예외적인 상황에서 일부러 맞춤법을 어기려는 컨셉의 문제에서는 그렇게 하지 않아도 됩니다. 또한, 맞춤법을 지키지 않는 표현이 맞춤법을 지키는 표현보다 매우 널리 사용되는 경우에도 맞춤법을 지키지 않아도 됩니다.
   ...
   **예 2)** 외래어 표기법에 따르면 ‘damage’는 ‘대미지’와 같이 적습니다. 단, ‘데미지’라는 표현이 널리 통용되기 때문에 ‘데미지’와 같이 작성해도 됩니다.
...
5. 수식과 수학 기호들은 문제와 참가자 수준에 맞도록 사용하는 것을 권장합니다. 디스크립션이 너무 길고 수식을 사용함으로써 디스크립션을 훨씬 더 간결하게 만들 수 있다면, 디스크립션이나 노트, 힌트 등에 수식에 사용한 기호에 대한 정의를 제공합니다.
...
1. 한 번만 입력받는 변수는 대문자, 이외의 다른 변수는 소문자로 정하는 것을 권장합니다.
   ...
   **예 2)** 이어서 *E*개의 줄에 걸쳐 간선을 이루는 두 점 *a*와 *b*가 공백을 사이에 두고 주어진다.
...
2. 변수의 범위 제한은 **[입력] 섹션**에서 해당 상수나 변수가 처음 등장하는 위치에 적는 것을 권장합니다. 가능한 경우 제한은 부등호를 사용한 수식으로 적는 것을 권장합니다. 범위 제한을 여러 개 적을 경우에는 세미콜론(;)으로 구분합니다.
   ...
   **예 1)** 첫째 줄에 세 수 *A*, *B*, *C*가 공백을 사이에 두고 주어진다. (1 ≤ *A*, *B* ≤ 1&thinsp;000&thinsp;000; 1 ≤ *C* ≤ 10<sup>9</sup>)

   **예 2)** 첫째 줄에 블록을 놓은 횟수 *N*(1 ≤ *N* ≤ 10&thinsp;000)이 주어진다.

3. 수는 3자리씩 공백으로 끊어 십진법으로 표기합니다. 수의 십진법 표기의 마지막 일곱 자리가 모두 0인 경우, 밑을 10으로 한 지수를 이용해 과학적 표기법으로 표기하는 것을 권장합니다. 수가 문제를 푸는 데에 특별한 의미를 갖는 경우, 괄호 안에 넣어 등호(=) 기호와 함께 표기하는 것을 권장합니다. 수 뒤의 조사는 수를 표시된 대로 한국어로 읽었을 때를 기준으로 붙입니다.

   **예 1)** 전체 경우의 수가 10<sup>7</sup>(십의 일곱제**곱**)을 넘지 않음을 증명할 수 있다.

   **예 2)** 첫 반례는 906&thinsp;150&thinsp;257(...이백오십**칠**)로 알려져 있다.

   **예 3)** 결과를 1&thinsp;000&thinsp;000&thinsp;007(= 10<sup>9</sup> + 7)로 나눈 나머지를 출력한다. 1&thinsp;000&thinsp;000&thinsp;007은 소수이다.

   **예 4)** 결과를 998&thinsp;244&thinsp;353(= 119 &times; 2<sup>23</sup> + 1)으로 나눈 나머지를 출력한다. 998&thinsp;244&thinsp;353은 소수이다.
...
2. 입출력 형식을 줄마다 설명하는 경우, ‘첫째 줄’, ‘둘째 줄’, ‘셋째 줄’, ‘*i*째 줄’ 등과 같이 표기하는 것을 권장합니다. ‘첫 번째 줄’, ‘두 번째 줄’, ‘세 번째 줄’, ‘*i*번째 줄’ 등과 같이 표기해도 됩니다.

   **예 1)** 첫째 줄에 정수 $N$이 주어진다. ($1 \leq N \leq 200\,000$)

   **예 2)** $i$번째 줄에는 $i$번 간선이 연결하는 두 정점 번호가 공백을 사이에 두고 주어진다.

3. 입출력 형식을 예를 들어 명시하는 경우, 예시는 본문과 명확히 구분될 수 있도록 포매팅을 다르게 합니다.

   **예 1)** 가능한 경우가 있으면 `YES`, 없으면 `NO`를 출력한다.

   **예 2)** 쿼리는 `a i j k`와 같은 형식으로 들어온다.

4. 한 줄에 여러 개의 정보를 작성하는 경우, 입력 형식에 공백으로 구분하였는지 아닌지를 명시합니다. 공백으로 구분한 경우 ‘공백을 사이에 두고 주어진다’와 같이 적는 것을 권장합니다.

   **예 1)** 첫째 줄에 가로 길이 *N*과 세로 길이 *M*이 공백을 사이에 두고 주어진다.

   **예 2)** 둘째 줄에 높이를 나타내는 한 자리 정수 *N*개가 공백 없이 주어진다.

   **예 3)** 셋째 줄에 가격을 나타내는 *A*개의 정수가 공백을 사이에 두고 주어진다.
...
1. 대회에 제공되는 문제의 포매팅을 TeX 형식 혹은 HTML 형식 중 한 가지로 통일합니다. TeX 형식과 HTML 형식은 아래에서 설명합니다.
2. 복잡한 수식이 없는 경우 모든 문제에 HTML 형식을 적용하는 것을 권장합니다.
3. 한 문제라도 TeX 렌더 엔진을 사용하는 경우 일관성을 위해 대회의 모든 문제에 TeX 렌더 엔진을 사용합니다. 이 경우 모든 수식은 TeX 섹션의 포매팅 컨벤션을 따릅니다.
...
5. 입력 제한을 시작하거나 끝내는 괄호 `(`, `)`, 나열된 제한을 구분하는 `;` 문자는 수식에 포함하지 않는 것을 권장합니다. 단, 우선순위를 높이거나 순서쌍을 나타내는 괄호는 수식에 포함합니다.

      | 올바른 예                                            | 잘못된 예                                          |
   | ---------------------------------------------------- | -------------------------------------------------- |
   | `($1 \le A, B \le 1\,000\,000$; $1 \le C \le 10^9$)` | `$(1 \le A, B \le 1\,000\,000;$ $1 \le C \le 10^9)$` |
   | `$i$번 도시와 $(i+1)$번 도시를 잇는 도로는 양방향입니다.` | `$i$번 도시와 ($i+1$)번 도시를 잇는 도로는 양방향입니다.` |

6. 순서가 있는 리스트는 enumerate, 순서가 없는 리스트는 itemize를 사용합니다.

7. sin, cos, max, lim 등은 그대로 적지 말고 각각의 함수에 맞는 operator를 사용합니다. 각각 \sin, \cos, \max, \lim 등입니다.

8. 수식의 ‘...’ 기호는 그대로 적지 않습니다. 대신 \cdots를 사용합니다.

9. 수식 안에 텍스트를 써야 할 경우에는 \text를 사용합니다.

10. 인라인 수식에서 \displaystyle(\dfrac 포함)을 사용하지 않는 것을 권장합니다. 인라인 수식이 너무 복잡해 문제의 가독성을 해칠 경우에는, 문맥을 적절히 수정하여 블록 수식으로 적는 것을 권장합니다.

## HTML

1. 모든 변수는 기울임꼴로 표시하는 것을 권장합니다. 숫자와 연산자는 어떤 경우에도 기울임꼴로 표시하지 않습니다. 필요에 따라 벡터, 행렬, 텐서의 경우, 굵은 기울임꼴로 표시해도 됩니다.
...
2. 연산자와 문자, 혹은 연산자와 수 사이에는 공백을 포함합니다.
...
4. 유니코드에 사용 가능한 기호가 있다면, 올바른 기호를 선택해서 사용합니다.
   ...
   | &#x20a9; `&#x20a9;`                 | \ (리터럴이 아닌 경우)  |
   | 10&thinsp;000 `10&thinsp;000`       | 10 000 `10 000`         |
